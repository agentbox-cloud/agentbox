...
import { spawnConnectedSSHTerminal } from 'src/sshTerminal'
...
export const spawnCommand = new commander.Command('spawn')
  ...
  .option('--custom', 'mark this as a custom template build.')
  .alias('sp')
  .action(
    async (
...
export async function connectSandbox({
  ...
  try {
    ...
    const lowerSandboxId = sandboxId.toLowerCase();
    if (lowerSandboxId.includes('brd')) {
      await spawnConnectedSSHTerminal(sandbox)
    } else {
...
export async function connectAgentBoxSandbox({
  apiKey,
  template,
}: {
  apiKey: string
