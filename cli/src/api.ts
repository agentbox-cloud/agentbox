...
const authErrorBox = (keyName: string) => {
  ...
  switch (keyName) {
    case 'E2B_API_KEY':
      link = 'https://agentbox.space/dashboard?tab=keys'
      msg = 'API key'
      break
    case 'E2B_ACCESS_TOKEN':
      link = 'https://agentbox.space/dashboard?tab=personal'
      msg = 'access token'
      break
  }
  ...
  return boxen.default(
    `You must be logged in to use this command. Run ${asBold('agentbox auth login')}.
...
export function ensureUserConfig(): UserConfig {
  const userConfig = getUserConfig()
  if (!userConfig) {
    console.error('No user config found, run `agentbox auth login` to log in first.')
    process.exit(1)
  }
  return userConfig
}
