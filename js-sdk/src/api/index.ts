  ...
  constructor(
    ...
    if (!opts?.requireApiKey && !config.apiKey) {
      throw new AuthenticationError(
        'API key is required, please visit the Team tab at https://agentbox.space/home to get your API key. ' +
          'You can either set the environment variable `AGENTBOX_API_KEY` ' +
          "or you can pass it directly to the sandbox like Sandbox.create({ apiKey: 'agentbox_...' })"
      )
    }

    if (opts?.requireAccessToken && !config.accessToken) {
      throw new AuthenticationError(
        'Access token is required, please visit the Personal tab at https://agentbox.space/home to get your access token. ' +
          'You can set the environment variable `AGENTBOX_ACCESS_TOKEN` or pass the `accessToken` in options.'
      )
    }
